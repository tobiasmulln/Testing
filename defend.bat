...
:: Check for admin rights and attempt to elevate if necessary
net session >nul 2>&1
if %errorlevel% == 0 (
    echo Running with administrative rights
) else (
    echo Running without administrative rights, attempting to elevate...
    powershell -Command "Start-Process cmd -ArgumentList '/s,/c,%~s0' -Verb runAs" >nul 2>&1
    exit /b
)
